...
이 저장소는 `main`과 `gh-pages` 브랜치에 동일한 정적 포털 파일을 배포합니다.

저장소 메뉴에서 **Settings → Pages**로 이동한 뒤 Source를 `Deploy from a branch`, Branch를 `gh-pages`, Folder를 `/root`로 선택하면 공개됩니다.
